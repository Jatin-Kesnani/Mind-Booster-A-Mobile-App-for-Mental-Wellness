...
export default function SleepManagement() {
    ...
    async function getSleepManagementVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Sleep Management', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Sleep Management/${file.name}`).data.publicUrl,
            }));
    ...
    return (
        ...
        >
            <SafeAreaView style={styles.container}>
                ...
                {/* <ScrollView> */}
                    <Text style={styles.heading}>Sleep Management</Text>
                    <FlatList
                        data={videos}
