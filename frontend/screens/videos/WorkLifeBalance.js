...
export default function WorkLifeBalance() {
    ...
    async function getWorkLifeBalanceVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Work-Life Balance', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Work-Life Balance/${file.name}`).data.publicUrl,
            }));
    ...
    return (
        ...
        >
            <SafeAreaView style={styles.container}>
                ...
                {/* <ScrollView> */}
                    <Text style={styles.heading}>Work-Life Balance</Text>
                    <FlatList
                        data={videos}
