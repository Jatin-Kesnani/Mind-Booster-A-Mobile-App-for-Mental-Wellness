...
export default function SocialAnxiety() {
    ...
    async function getSocialAnxietyVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Social Anxiety', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Social Anxiety/${file.name}`).data.publicUrl,
            }));
    ...
    return (
        ...
        >
            <SafeAreaView style={styles.container}>
                ...
                {/* <ScrollView> */}
                    <Text style={styles.heading}>Social Anxiety</Text>
                    <FlatList
                        data={videos}
