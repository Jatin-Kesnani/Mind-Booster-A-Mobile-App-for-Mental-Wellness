...
export default function TimeManagement() {
    ...
    async function getTimeManagementVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Time Management', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Time Management/${file.name}`).data.publicUrl,
            }));
    ...
    return (
        ...
        >
            <SafeAreaView style={styles.container}>
                ...
                {/* <ScrollView> */}
                    <Text style={styles.heading}>Time Management</Text>
                    <FlatList
                        data={videos}
