...
export default function FinancialStress() {
    ...
    async function getFinancialStressVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Financial Stress', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Financial Stress/${file.name}`).data.publicUrl,
            }));
