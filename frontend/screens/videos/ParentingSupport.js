...
export default function ParentingSupport() {
    ...
    async function getParentingSupportVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Parenting Support', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Parenting Support/${file.name}`).data.publicUrl,
            }));
    ...
    return (
        ...
        >
            <SafeAreaView style={styles.container}>
                ...
                {/* <ScrollView> */}
                    <Text style={styles.heading}>Parenting Support</Text>
                    <FlatList
                        data={videos}
