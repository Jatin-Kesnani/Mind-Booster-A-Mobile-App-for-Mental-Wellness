...
export default function AngerManagement() {
    ...
    async function getAngerManagementVideos() {
        const { data, error } = await supabase
            .storage
            .from('videos')
            .list('Anger Management', { limit: 10, offset: 0 });
        ...
        if (error) {
            ...
            const videoUrls = data.map((file) => ({
                ...
                url: supabase.storage
                    .from('videos')
                    .getPublicUrl(`Anger Management/${file.name}`).data.publicUrl,
            }));
    ...
    return (
        ...
        >
            <SafeAreaView style={styles.container}>
                ...
                {/* <ScrollView> */}
                    <Text style={styles.heading}>Anger Management</Text>
                    <FlatList
                        data={videos}
